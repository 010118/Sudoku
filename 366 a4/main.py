...
class AC3:
    ...
    def pre_process_consistency(self, grid):
        ...
        return self.consistency(grid, temp)
    ...
    def consistency(self, grid, Q):
        ...
        while len(temp) > 0:
            lista = temp.pop()
            assigned_row, failure = self.remove_domain_row(grid, lista[0], lista[1])
            if failure: 
                return failure

            assigned_column, failure = self.remove_domain_column(grid, lista[0], lista[1])
            if failure: 
                return failure

            assigned_unit, failure = self.remove_domain_unit(grid, lista[0], lista[1])
            if failure: 
                return failure


            temp.extend(assigned_row)
            temp.extend(assigned_column)
            temp.extend(assigned_unit)
        return False
    ...
    def search(self, grid, var_selector):
        ...
        for d in grid.get_cells()[var[0]][var[1]]:
            ...
            if grid.is_value_consistent(d, var[0], var[1]):
                g = grid.copy()
                g.get_cells()[var[0]][var[1]] = d

                a = AC3()
                b = a.consistency(g,var)

                if not b:
                    rb, j = self.search(g, var_selector)
                    if j:
                        return rb, True
        return None, False
...
for p in problems:
    ...
    AC3().pre_process_consistency(temp_MRV)
    sol, res = Backtracking().search(temp_MRV, MRV())
    sol.print()
    print("Using MRV")
    ...
    FA_start = time.time()
    AC3().pre_process_consistency(temp_FA)
    sol, res = Backtracking().search(temp_FA, FirstAvailable())
    sol.print()
    print("Using FA")
    print('Is the current grid a solution? ', sol.is_solved())
    FA_end = time.time()
